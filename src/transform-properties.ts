...
export function transformProperties(
  decl: Declaration,
  customProperties: Map<string, valuesParser.ParsedValue>,
  opts: { preserve?: boolean; matchers: RegExp[]; includeVars: boolean },
): void {
  if (isTransformableDecl(decl, opts) && !isDeclarationIgnored(decl)) {
    ...
    if (opts.preserve) {
      ...
      if (hasTrailingComment(beforeDecl) && beforeDecl.raws?.value) {
        beforeDecl.raws.value.value = beforeDecl.value.replace(
          TRAILING_COMMENT_REGEX,
          '$1',
        )
        beforeDecl.raws.value.raw =
          beforeDecl.raws.value.value +
          beforeDecl.raws.value.raw.replace(TRAILING_COMMENT_REGEX, '$2')
      }
    } else {
      decl.value = value
      ...
      if (hasTrailingComment(decl) && decl.raws?.value) {
        ...
        decl.raws.value.raw =
          decl.raws.value.value +
          decl.raws.value.raw.replace(TRAILING_COMMENT_REGEX, '$2')
      }
    }
  }
}
...
const isTransformableDecl = (
  decl: Declaration,
  opts: { preserve?: boolean; matchers: RegExp[]; includeVars: boolean },
): boolean =>
  (opts.includeVars || !decl.variable) &&
  decl.value.includes('--') &&
  decl.value.toLowerCase().includes('var(')

// whether the declaration has a trailing comment
// eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
const hasTrailingComment = (decl: Declaration): boolean =>
  'value' in Object(Object(decl.raws).value) &&
  'raw' in (decl.raws?.value ?? {}) &&
  TRAILING_COMMENT_REGEX.test(decl.raws.value?.raw ?? '')
...
function parentHasExactFallback(decl: Declaration, value: string): boolean {
  ...
  decl.parent.each((sibling, index) => {
    ...
    if (
      sibling.prop.toLowerCase() === decl.prop.toLowerCase() &&
      sibling.value === value
    ) {
      hasFallback = true
    }
