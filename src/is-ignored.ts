...
const blockRegExp = /(?:!\s*)?postcss-inline-properties:\s*off\b/i
...
const DECLARATION_REG_EXP =
  /(?:!\s*)?postcss-inline-properties:\s*ignore\s+next\b/i
...
function isIgnoreComment(node: Node | undefined, regexp: RegExp): boolean {
  return (
    !!node && node.type === 'comment' && regexp.test((node as Comment).text)
  )
}
